...
- 在技能计划页底部的“职业数据管理”中，“导出 JSON”会下载当前账号的简历、岗位、投递、面试（含问题）、报告和技能记录。服务端会一次性在内存中组装全部数据；数据量很大时导出可能较慢或失败，此时请避免重复点击并联系部署方评估容量。
- “清空职业数据与知识库”要求输入大写 `DELETE`，会删除上述职业记录和当前用户的 Chroma 知识库。
